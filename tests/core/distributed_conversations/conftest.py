...
async def create_controller(name, network=None):
...
@pytest.fixture
def two_controllers(reset_store):
    async def get_controllers():
        ctrl1 = await create_controller('ctrl1')
        ctrl2 = await create_controller('ctrl2', ctrl1.pusher.network)
